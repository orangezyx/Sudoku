...
class SudokuGame(object):
    def __init__(self):
        ...
        cols = curses.COLS
        self.game_frm.addstr(5, 0, '+---Choose the difficult---+'.center(cols, ' '))
        self.game_frm.addstr(6, 0, '|         1. Easy          |'.center(cols, ' '))
        self.game_frm.addstr(7, 0, '|         2. Normal        |'.center(cols, ' '))
        self.game_frm.addstr(8, 0, '|         3. Hard          |'.center(cols, ' '))
        self.game_frm.addstr(9, 0, '|         4. Expert        |'.center(cols, ' '))
        self.game_frm.addstr(10, 0, '|         q. Exit          |'.center(cols, ' '))
        self.game_frm.addstr(11, 0, '+--------------------------+'.center(cols, ' '))
    ...
    def game_loop(self, diff):
        def generate(clean_nums): # 生成数独
            ...
            for i in range(0, 7, 2):
                switchColumns(randomColumns[i],\
                            randomColumns[i + 1])
            # 随机清空格子
            positions = {(x % 9, int(x / 9)) for x in random.sample(range(0, 81), 81 - clean_nums)}
        ...
        self.board = generate(n) # 生成带几个数字的 board
        ...
        notify_text_2 = '[r] restart     [s] save     [<-] clear current num'

        for r in range(len(base_ui)):
            self.game_frm.addstr(5 + r, 0, base_ui[r].center(curses.COLS))
        self.game_frm.addstr(5 + len(base_ui) + 1, 0, notify_text_1.center(curses.COLS))
        self.game_frm.addstr(5 + len(base_ui) + 2, 0, notify_text_2.center(curses.COLS))
        self.game_frm.getch()