...
mainWindow = tk.Tk()
mainWindow.title('Sudoku GUI Application')
...
for i in range(9):
    for j in range(9):
        ent = tk.Entry(f1, text='', width=2, font=('Helvetica', '20', 'bold'))
        texts.append(ent)
...
for i in range(9):
    for j in range(9):
        lab = tk.Label(f2, text='0', width=2, font=('Helvetica', '20', 'bold'))
        labels.append(lab)
...
def click():
    ...
    grid = [list(new_int(t.get()) for t in texts[i*9:i*9+9]) for i in range(9)]
    s = Sudoku(grid)
    r = s.solve()
    if r.startswith('+'):
